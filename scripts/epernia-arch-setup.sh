...
echo "==================================================="
echo " CIAA-Launcher Automatic Installer for Arch Linux  "
echo "==================================================="
...
if command -v pacman &>/dev/null; then
    sudo pacman -Sy --noconfirm jq unzip openocd 
elif command -v apt &>/dev/null; then
    sudo apt update && sudo apt install -y jq unzip openocd
elif command -v dnf &>/dev/null; then
    sudo dnf install -y jq unzip openocd
else
    echo "⚠ Could not detect a supported package manager. Please install 'jq' and 'unzip' manually."
fi

# 2. Download CIAA-Launcher and extract immediately
...
# 3. Download and extract firmware_v3
echo "[2/6] Downloading firmware_v3..."
cd $USER_HOME/CIAA
...
# 5. Download Embedded IDE templates
...
# 6. Replace existing install-tools.sh with Arch Linux version
echo "[6/6] Setting up Arch Linux specific tools..."
set -e

# User to run AUR commands
USERNAME="${SUDO_USER:-$(whoami)}"

# Official packages (require root)
OFFICIAL_PACKAGES="git base-devel"

# AUR packages (install as normal user)
AUR_PACKAGES="gtkterm cutecom gconf fuse gksu"
...
if ! command -v paru >/dev/null 2>&1; then
    ...
    echo "[*] Installing AUR packages as user $USERNAME..."
...
echo "[*] Configuring permissions for serial ports..."
usermod -aG uucp,lock "$USERNAME"

bash /home/$USERNAME/CIAA/embedded-ide/usr/bin/ftdi-tools.sh --install

# ---------- Load FTDI kernel module ----------
...
if lsmod | grep -q "^ftdi_sio"; then
    ...
    if modinfo ftdi_sio &>/dev/null; then
        ...
        echo "Make sure you have linux-firmware and kernel modules installed."
    fi
fi
...
# ---------- OpenOCD fix for Arch Linux ----------
echo "[*] Applying OpenOCD fix for Arch Linux..."
mv $USER_HOME/CIAA/tools/openocd/bin/librt.so.1 $USER_HOME/CIAA/tools/openocd/bin/librt.so.1.bak
